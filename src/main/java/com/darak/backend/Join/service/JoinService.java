...
public class JoinService {
    ...
    public void Join_Save(JoinSaveDto joinSaveDto) throws Exception {
        if(joinRepository.findByItem_ItemNameAndItem_StoreName(joinSaveDto.getItem_name(), joinSaveDto.getStore_name()).isPresent()) {
    ...
    public JoinResponseDto Join_select (JoinRequestDto joinRequestDto) throws Exception {
        Optional<Join> join = joinRepository.findByItem_ItemNameAndItem_StoreName(joinRequestDto.getItemName(), joinRequestDto.getStoreName());
        if(join.isEmpty()) {
            throw new Exception("공구 정보가 없는 아이템입니다!");
        }
        Join join1 = join.get();
        return new JoinResponseDto(joinRequestDto.getItemName(),joinRequestDto.getStoreName(),join1.getJoinPrice(),join1.getJoinLimit(),join1.getJoinPercent(),join1.getJoinStartDay(),join1.getJoinEndDay());
    }
    @Transactional
    public void join_people_update(JoinRequestDto joinRequestDto) throws Exception {
        Optional<Join> join =joinRepository.findByItem_ItemNameAndItem_StoreName(joinRequestDto.getItemName(), joinRequestDto.getStoreName());
        Join join1 = join.get();
        join1.changeJoinLimit(join1.getJoinLimit()+1);
    }
}
